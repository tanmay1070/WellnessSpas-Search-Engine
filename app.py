...
def get_clustering_results(clust_inp, param_type):
    ...
    if param_type == "flat_clustering":
        ...
        f = open('clustering/test_single_clustering_result_20k.txt')
        lines = f.readlines()
        f.close()
    
    elif param_type == "dummy_clustering":
        f = open('clustering/Complete_clustering.txt')
        lines = f.readlines()
        f.close()
...
def get_hits_results(clust_inp):
    authority_score_file = open("HITS/precomputed_scores/authority_score", 'r').read()
